<template>
  <main>
    ...
    <div class="themes-grid">
      <div
        v-for="theme in sortedThemesWithMedia"
        :key="theme.id"
        class="theme-tile"
...
async function fetchThemesWithMedia() {
  ...
  if (res.ok) {
    // [{id, theme_name, max_media_id}]
    themesWithMedia.value = await res.json()
  }
}

const sortedThemesWithMedia = computed(() =>
  [...themesWithMedia.value].sort((a, b) => (b.max_media_id || 0) - (a.max_media_id || 0))
)

function goToThemeMaterials(themeId) {
  router.push(`/media/materials/${themeId}`)
}
...
onMounted(fetchThemesWithMedia)
</script>

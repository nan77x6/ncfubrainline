<template>
  <main>
    <h1>Медиа по предметам</h1>
    <div class="media-grid">
      <div
        v-for="subject in sortedSubjectsWithMedia"
        :key="subject.id"
        class="media-tile"
        @click="goToSubjectMedia(subject.id)"
      >
...
async function fetchSubjectsWithMedia() {
  ...
  if (res.ok) {
    // Ожидаем, что бэкенд вернёт [{id, subject_name, max_media_id}]
    subjectsWithMedia.value = await res.json()
  }
}

const sortedSubjectsWithMedia = computed(() =>
  [...subjectsWithMedia.value].sort((a, b) => (b.max_media_id || 0) - (a.max_media_id || 0))
)

function goToSubjectMedia(subjectId) {
  router.push(`/media/subject/${subjectId}`)
}

onMounted(fetchSubjectsWithMedia)
</script>
